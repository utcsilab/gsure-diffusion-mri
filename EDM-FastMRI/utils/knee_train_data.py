...
def task(i):
    ...
    var_noisy = np.var(cimg_white_noisy[:, 0:20, 0:20])
...
ksp_files = final_idx
x_est = torch.zeros(2000, 440, 368, dtype=torch.complex64)
x_est_gt = torch.zeros(2000, 440, 368, dtype=torch.complex64)
u_images = torch.zeros(2000, 440, 368, dtype=torch.complex64)
...
with Pool(n_proc) as p:
    for i, gt_img_white_cropped, gt_img_white_cropped_noisy, u_cropped_white_noisy, norm_const_99, var_noisy, ksp_white_noisy, s_maps_white_noisy in tqdm(p.imap(task, range(2000))):
