import torch
import numpy as np
from ops import A_forward, A_adjoint, A_normal
...
def conjgrad_single(x, b, mask, maps, max_iter=10, l2lam=0., eps=1e-4, verbose=True, complex=True, device='cpu'):
...
#     print('normal shape:',A_aug_normal(img=x, mask=masks, maps=maps, thetas=thetas).shape )
    r = b - (A_normal(image=x, maps=maps, mask = mask) + l2lam * x) #expects the shape [batch , H, W]
...
#         print('p shape:', p.shape)
        Ap = A_normal(image=p, maps=maps, mask=mask) + l2lam * p
        pAp = _dot_batch(p, Ap)
